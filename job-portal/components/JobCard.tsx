...
export default function JobCard({ job, applied, onApply, isRecruiter }: JobCardProps) {
  const expired = new Date(job.deadline) < new Date();
  return (
    <div className="card">
      <div className="card-title"><Link href={`/jobs/${job._id}`}>{job.title}</Link></div>
      ...
      {!isRecruiter && (
        applied ? (
          ...
          <button className="btn btn-primary btn-sm" onClick={() => onApply(job._id)} disabled={expired}>
