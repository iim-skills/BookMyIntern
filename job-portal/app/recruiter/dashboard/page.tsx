...
import type {
  IJob, IApplication, JobFormData, ApplicationStatus,
} from '@/types';
...
const REVIEWABLE: ApplicationStatus[] = [
...
export default function RecruiterDashboard() {
  ...
  // ── FIX: plain object instead of Set ─────────────────────────────────────
  ...
  const handleCreate = async (form: JobFormData) => {
    ...
    if (!res.ok) { flash(data.error ?? 'Error creating job.'); return; }
    setShowCreate(false);
    flash('Job posted!');
    void loadJobs();
  };
  ...
  const handleEdit = async (form: JobFormData) => {
    ...
    const data = await res.json() as { error?: string };
    setFormLoading(false);
    if (!res.ok) { flash(data.error ?? 'Error updating job.'); return; }
    setEditJob(null);
    flash('Job updated.');
    void loadJobs();
  };
  ...
  const handleDelete = async (jobId: string) => {
    ...
    const data = await res.json() as { error?: string };
    if (!res.ok) { flash(data.error ?? 'Error deleting job.'); return; }
    flash('Job deleted.');
    void loadJobs();
  };
  ...
  const viewApplicants = async (job: IJob) => {
    ...
    // Reset expanded state for the new applicant list
    setExpanded({});
  };
  ...
  if (!verified)
    return (
      <VerificationModal
        onVerified={() => { setVerified(true); void loadJobs(); }}
      />
    );
  ...
  if (applicants) return (
    <div className="dashboard">
      <div className="page-hd">
        <div>
          <button className="btn btn-sm" onClick={() => setApplicants(null)}>
            ← Back
          </button>
          <h1 style={{ marginTop: 10 }}>Applicants — {applicants.title}</h1>
        </div>
        <span style={{ fontSize: '0.82rem', color: '#6b7280' }}>
          {applicants.list.length} applicant
          {applicants.list.length !== 1 ? 's' : ''}
        </span>
      </div>
            ...
            <div className="card" key={a._id}>
              ...
              <div className="applicant-row">
                ...
                <div className="applicant-actions">
                  <span className={`tag ${STATUS_CLS[a.status] ?? ''}`}>
                    {a.status}
                  </span>
                  ...
                  {student && (
                    <button
                      ...
                      onClick={() =>
                        void messageStudent(student._id, applicants.jobId)
                      }
                    >
                  ...
                  {student && canReview && (
                    <Link
                      href={
                        `/reviews/write?studentId=${student._id}` +
  ...
  if (editJob) {
    const initial: Partial<JobFormData> = {
      ...
      deadline: editJob.deadline
        ? new Date(editJob.deadline).toISOString().slice(0, 10)
        : '',
  ...
  return (
    <div className="dashboard">
      <div className="page-hd">
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            ...
            onClick={() => { setShowCreate((s) => !s); setMsg(''); }}
          >
      ...
      {jobsLoading ? (
        ...
        <div className="empty">
          No jobs posted yet. Click &ldquo;+ Post New Job&rdquo; to start.
        </div>
      ) : (
        jobs.map((job) => (
          <div className="card" key={job._id}>
            ...
            <div className="job-actions">
              ...
              <Link href={`/jobs/${job._id}`} className="btn btn-sm">
                Preview ↗
              </Link>
            </div>
          </div>
